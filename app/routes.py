...
def index():
    ...
    next_url = url_for(index.__name__, page=posts.next_num) if posts.has_next else None
    prev_url = url_for(index.__name__, page=posts.prev_num) if posts.has_prev else None
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not next_page or url_parse(next_page).netloc != '':
            next_page = url_for(index.__name__)
...
def user_details(username):
    ...
    next_url = url_for(user_details.__name__, username=username, page=posts.next_num) if posts.has_next else None
    prev_url = url_for(user_details.__name__, username=username, page=posts.prev_num) if posts.has_prev else None
    form = EmptyForm()
...
def edit_profile():
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for(edit_profile.__name__))
...
def explore():
    ...
    next_url = url_for(explore.__name__, page=posts.next_num) if posts.has_next else None
    prev_url = url_for(explore.__name__, page=posts.prev_num) if posts.has_prev else None
